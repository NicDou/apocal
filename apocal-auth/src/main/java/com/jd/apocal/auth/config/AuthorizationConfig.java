...
import com.jd.apocal.auth.util.jackson2.AuthJackson2Module;
...
public class AuthorizationConfig extends AuthorizationServerConfigurerAdapter {
  ...
  public void configure(AuthorizationServerEndpointsConfigurer endpoints) {
    ...
    endpoints
        ...
        //开启密码模式 tokenGranters  ResourceOwnerPasswordGranter
        .authenticationManager(authenticationManager)
        .reuseRefreshTokens(false)
        //密码模式下username 和 password验证 可以不配？？
   ...
   * tokenstore 定制化处理
   *
   * @return TokenStore 1. 如果使用的 redis-cluster 模式请使用 VoleRedisTokenStore VoleRedisTokenStore
   * tokenStore = new VoleRedisTokenStore(); tokenStore.setRedisTemplate(redisTemplate);
   */
  @Bean
  ...
  public TokenEnhancer tokenEnhancer() {
    return (accessToken, authentication) -> {
      ...
      UserDetailsImpl user = (UserDetailsImpl) authentication
