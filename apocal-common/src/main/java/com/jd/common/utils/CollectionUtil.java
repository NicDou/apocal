...
public class CollectionUtil {
  ...
  public static Object[] subtractArray(Object[] sourceAry, int begin, int length) {
    ...
    if (begin == 0) {
      ...
      System.arraycopy(sourceAry, begin + length, distinctAry, begin,
          sourceAry.length - length - begin);
    }
    return distinctAry;
  }
  ...
  public static int judgeObjectDimen(Object obj) {
    ...
    if (obj instanceof Collection || obj.getClass().isArray() || obj instanceof Map) {
      ...
      if (obj instanceof Collection) {
        ...
        if (setItem.getClass().isArray() || setItem instanceof Collection
   ...
   * @todo 创建汇总行
   */
  private static List createSummaryRow(Object[] rowSummaryData, List rowList, int groupIndex,
      Object[] title,
    ...
    if (title[2] != null || (title[3].equals("left") || title[3].equals("right"))) {
      ...
      for (int i = 0, n = rowSummaryData.length; i < n; i++) {
        if (rowSummaryData[i] == null) {
          ...
          if (radixSize >= 0) {
            average
                .add(i, BigDecimal.valueOf(averageValue).setScale(radixSize, BigDecimal.ROUND_FLOOR)
                    .doubleValue());
          } else {
   ...
   * @todo 汇总计算
   */
  private static void calculateTotal(HashMap groupSumMap, List rowList, Integer[] summaryColumns,
      int radixSize) {
    Object[] groupSums;
