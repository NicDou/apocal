...
import com.jd.common.utils.StreamUtils;
...
/**
 * RSA加/解密 非对称算法；签名算法默认为SHA1WithRSA
 */
public class RSAEncrypter {
   ...
   * @param b64Key     自定义base64字符串(不用则填Null)
   * @param useHex     是否使用十六位字符表示(true则不使用Base64)
   * @param useMD5     是否使用MD5withRSA签名(默认SHA1WithRSA)
   */
  public RSAEncrypter(String publicKey, String privateKey, String b64Key, boolean useHex,
      boolean useMD5) {
    if (useMD5) {
    ...
    init(publicKey, privateKey,
        useHex ? null : StringUtils.isBlank(b64Key) ? B64Encrypter.DEFAULT_ALPHABET : b64Key);
  }

  /**
   * @param publicKey
   * @param privateKey
   * @param storeAlias    私钥文件user name
   * @param storePassword 私钥文件 member pass
   */
  private RSAEncrypter(InputStream publicKey, InputStream privateKey, String storeAlias,
      String storePassword) {
    init(publicKey, privateKey, storeAlias, storePassword, B64Encrypter.DEFAULT_ALPHABET);
  }

  private RSAEncrypter(InputStream publicKey, InputStream privateKey, String storeAlias,
      String storePassword, boolean useHex) {
    init(publicKey, privateKey, storeAlias, storePassword,
        useHex ? null : B64Encrypter.DEFAULT_ALPHABET);
  }

  private RSAEncrypter(InputStream publicKey, InputStream privateKey, String storeAlias,
      String storePassword, String b64Key) {
  ...
  private void init(String publicKey, String privateKey, String b64Key) {
    ...
    try {
      this.publicKey = KeyFactory.getInstance(KEY_ALGORITHM)
          .generatePublic(new X509EncodedKeySpec(decodeTransfer(publicKey)));
      this.privateKey = KeyFactory.getInstance(KEY_ALGORITHM)
          .generatePrivate(new PKCS8EncodedKeySpec(decodeTransfer(privateKey)));
  ...
  /**
   * @param publicKey  <br/> CER/CRT
   * @param privateKey <br/> 默认JSK
   */
  private void init(InputStream publicKey, InputStream privateKey, String storeAlias,
      String storePassword, String b64Key) {
    if (!StringUtils.isBlank(b64Key)) {
      b64Encrypter = B64Encrypter.getInstance(b64Key);
    }
  ...
  public static RSAEncrypter getInstance(String publicKey, String privateKey, String b64Key) {
    if (StringUtils.isBlank(privateKey) || StringUtils.isBlank(publicKey) || !B64Encrypter
        .checkAlphabet(b64Key)) {
      return null;
    }
    String key = new StringBuilder().append(privateKey).append("_").append(publicKey).append("_")
        .append(b64Key).toString();
  ...
  public static RSAEncrypter newInstance(InputStream publicKey, InputStream privateKey,
      String storeAlias, String storePassword) {
    return new RSAEncrypter(publicKey, privateKey, storeAlias, storePassword);
  }

  public static RSAEncrypter newInstance(InputStream publicKey, InputStream privateKey,
      String storeAlias, String storePassword, boolean useHex) {
    return new RSAEncrypter(publicKey, privateKey, storeAlias, storePassword, useHex);
  }

  public static RSAEncrypter newInstance(InputStream publicKey, InputStream privateKey,
      String storeAlias, String storePassword, String b64Key) {
