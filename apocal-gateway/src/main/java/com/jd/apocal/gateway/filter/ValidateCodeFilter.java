...
public class ValidateCodeFilter extends ZuulFilter {
  ...
  public boolean shouldFilter() {
    ...
    if (!StringUtils.containsIgnoreCase(request.getRequestURI(),
        SecurityConstants.OAUTH_TOKEN_URL) || !StringUtils
        .containsIgnoreCase(request.getRequestURI(), SecurityConstants.MOBILE_TOKEN_URL)) {
      return false;
    }
    ...
    try {
      ...
      if (CollectionUtils
          .containsAny(filterIgnorePropertiesConfig.getClients(), Arrays.asList(clientInfos))) {
        return false;
      }
    } catch (IOException e) {
  ...
  public Object run() {
    try {
      ...
      RestServiceResult<String> result = new RestServiceResult<>(e);
      result.setCode(478);
