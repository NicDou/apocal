package com.jd.apocal.gateway.util.jackson2;
...
public class DefaultSavedRequestDeserializer extends JsonDeserializer<DefaultSavedRequest> {
  ...
  @Override
  public DefaultSavedRequest deserialize(JsonParser jp, DeserializationContext ctxt)
    ...
    ArrayList<SavedCookie> cookies = mapper
        .convertValue(jsonNode.get("cookies"), new TypeReference<ArrayList<SavedCookie>>() {
        });
    List<Locale> locales = mapper
        .convertValue(jsonNode.get("locales"), new TypeReference<List<Locale>>() {
        });
    Map<String, List<String>> headers = mapper
        .convertValue(jsonNode.get("headers"), new TypeReference<Map<String, List<String>>>() {
        });
    Map<String, String[]> parameters = mapper
        .convertValue(jsonNode.get("parameters"), new TypeReference<Map<String, String[]>>() {
        });
    DefaultSavedRequestDeserializer.Builder builder =
        new DefaultSavedRequestDeserializer.Builder(contextPath.asText(), method.asText(), pathInfo,
            queryString,
            requestURI.asText(), requestURL.asText(), scheme.asText(), serverName.asText(),
            servletPath.asText(), serverPort.asInt(), cookies, locales, headers, parameters);
  ...
  class Builder extends DefaultSavedRequest.Builder {
    ...
    public Builder(String contextPath, String method, String pathInfo, String queryString,
        String requestURI, String requestURL, String scheme, String serverName, String servletPath,
        int serverPort, ArrayList<SavedCookie> cookies, List<Locale> locales,
